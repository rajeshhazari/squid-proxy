...
// login method
loginRouter.post('/', (req: Request, response: Response, next: NextFunction) => {
  const agent = req.header('user-agent');   // "Mozilla/5.0 (Macintosh Intel Mac OS X 10_8_5) AppleWebKi..."
  const authToken  = req.header('Authorization'); // "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..."
  pbkdf2(req.body.password, user.salt, 10000, length, digest, (err: Error, hash: Buffer) => {
    if (err) {
